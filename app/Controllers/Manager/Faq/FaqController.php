<?php
namespace App\Controllers\Manager\Faq;
...
class FaqController extends Controller
{
  ...
  public function index()
  {
    ...
    return view()->make('manager.faqs.index');
  }
  ...
  public function create()
  {
  	...
  	if ( request()->isMethod('post') )
  	{
  		...
  		if ( $validator->fails() ) {
        request()->flash();
        return view()->make('manager.faqs.form')->withInput($input)->withErrors($validator);
  		}
  		else {
        $testimonial = $this->page->getModel();
        $testimonial->title = $input['title'];
        $testimonial->description = $input['description'];
        $testimonial->sort_order = $input['sort_order'];
        $testimonial->type = 'faq';

  			if ( $testimonial->save() ) {
          $this->handle_image( $testimonial );
  				session()->flash('message', 'Faq ' . $input['title'] . ' is added');
  			}
  		}
  	}
  	
  	return view()->make('manager.faqs.form');
  }
  	...
  	if ( request()->isMethod('post') )
  	{
        ...
        if ( $data->save() ) {
          $this->handle_image( $data );
  				session()->flash('message', 'Faq ' . $input['title'] . ' is updated');
  			}
  		}
  	}

  	return view()->make('manager.faqs.form', compact('data'));
  }
  ...
  protected function list_action_button( $row )
  {
    ...
    <div class="btn-group">
      ...
      <ul class="dropdown-menu dropdown-menu-list">
        <li><a class="action-edit" href="<?php echo route('admin.faqs.update', ['id' => $row->id]) ?>"><i class="fa falist fa-edit"></i>Edit</a></li>
        <li><a class="action-delete" href="#" data-id="<?php echo $row->id?>" data-name="'<?php echo $row->title?>'"><i class="fa falist fa-trash"></i>Delete</a></li>
      </ul>
    </div>
    <?php
